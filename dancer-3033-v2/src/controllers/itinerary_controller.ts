...
export class ItineraryController {
  public routes(app: any): void {
    ...
    app
      .route("/getItinerariesByAssociation")
      .post(async (req: Request, res: Response) => {
        const msg = `🌽🌽🌽 getItinerariesByAssociation requested `;
        console.log(msg);
        try {
    ...
    app
      .route("/getItinerariesByDestinationCity")
      .post(async (req: Request, res: Response) => {
        const msg = `🌽🌽🌽 getItinerariesByDestinationCity requested `;
        console.log(msg);
        try {
          const itineraries = await Itinerary.find({
            "destinationCities.cityID": req.body.cityID,
          });
          res.status(200).json(itineraries);
        } catch (err) {
          res.status(400).json({
    ...
    app
      .route("/getItinerariesByRoute")
      .post(async (req: Request, res: Response) => {
        const msg = `🌽🌽🌽 getItinerariesByRoute requested `;
        console.log(msg);
        try {
          const itineraries = await Itinerary.find({
    ...
    app
      .route("/getItinerariesByLandmark")
      .post(async (req: Request, res: Response) => {
        const msg = `🌽🌽🌽 getItinerariesByLandmark requested `;
        console.log(msg);
        try {
          const itineraries = await Itinerary.find({
            "landmarks.landmarkID": req.body.landmarkID,
          });
          res.status(200).json(itineraries);
        } catch (err) {
          res.status(400).json({
    ...
    app
      .route("/findItinerariesByLocation")
      .post(async (req: Request, res: Response) => {
        ...
        try {
          ...
          log(
            `🔆🔆🔆 findItinerariesByLocation: elapsed time: 💙 ${
              end / 1000 - now / 1000
            } 💙 seconds for query`
          );
          res.status(200).json(result);
