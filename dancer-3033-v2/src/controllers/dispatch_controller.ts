  ...
  public routes(app: any): void {
    ...
    app
      ...
      .post(async (req: Request, res: Response) => {
        try {
          if (!req.body.routeID) {
            const msg =
              "DispatchController: 🍎 Dispatch recording failed: Missing route info";
            console.error(msg);
            res.status(400).json({
              message: msg,
            });
          ...
          if (!req.body.vehicleID) {
            const msg =
              "DispatchController: 🍎 Dispatch recording failed: Missing vehicle info";
            console.error(msg);
            res.status(400).json({
              message: msg,
            });
          ...
          if (!req.body.landmarkID) {
            const msg =
              "DispatchController: 🍎 Dispatch recording failed: Missing landmark info";
            console.error(msg);
            res.status(400).json({
              message: msg,
            });
          ...
          if (req.body.vehicleArrivalID) {
            const arrival: any = await VehicleArrival.findOne({
              vehicleArrivalID: req.body.vehicleArrivalID,
            });
            if (arrival) {
          ...
          console.error(
            "DispatchController: 💦 Dispatch recording failed",
            err
          );
          res.status(400).json({
            error: err,
    ...
    app.route("/addTripComplete").post(async (req: Request, res: Response) => {
      try {
        ...
        if (!tripComplete) {
          const msg =
            "DispatchController: 🍎 tripComplete failed: Missing trip parameters";
          console.error(msg);
          res.status(400).json({
            message: msg,
          });
    ...
    app.route("/addTripStart").post(async (req: Request, res: Response) => {
      try {
        ...
        if (!tripStart) {
          const msg =
            "DispatchController: 🍎 tripStart failed: Missing trip parameters";
          console.error(msg);
          res.status(400).json({
            message: msg,
          });
    ...
    app
      .route("/getDispatchRecordsByLocation")
      .post(async (req: Request, res: Response) => {
        try {
    ...
    app
      .route("/getDispatchRecordsByLandmark")
      .post((req: Request, res: Response) => {
        try {
          const days = req.body.days;
          const cutOff: string = moment().subtract(days, "days").toISOString();
    ...
    app
      .route("/getDispatchRecordsByMarshal")
      .post((req: Request, res: Response) => {
        try {
          const days = req.body.days;
          const cutOff: string = moment().subtract(days, "days").toISOString();
          const result = DispatchRecord.find({
            marshalID: req.body.marshalID,
            created: { $gt: cutOff },
          }).exec(function (err, records) {
            log(
              `🥦 🥦 🥦 getDispatchRecordsByMarshal:found  🍎 ${records.length} DispatchRecords for marshal: ${req.body.marshalID}`
            );
    ...
    app
      .route("/getDispatchRecordsByRoute")
      .post((req: Request, res: Response) => {
        try {
          const days = req.body.days;
          const cutOff: string = moment().subtract(days, "days").toISOString();
    ...
    app
      .route("/getDispatchRecordsByOwner")
      .post((req: Request, res: Response) => {
    ...
    app
      .route("/addMarshalFenceDwellEvent")
      .post(async (req: Request, res: Response) => {
        try {
    ...
    app
      .route("/addMarshalFenceExitEvent")
      .post(async (req: Request, res: Response) => {
        try {
